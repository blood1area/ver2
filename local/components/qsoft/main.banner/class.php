...
class AdvertisingBanner extends \CBitrixComponent
{
	...
	public function onPrepareComponentParams($params)
	{
		...
		if (isset($params['QUANTITY']) && intval($params['QUANTITY']) > 0 && $USER->IsAuthorized())
		{
		...
		if (!empty($this->arBanners) && is_array($this->arBanners))
		{
			foreach ($this->arBanners as $key => $banner)
			{
				...
				else
				{
					...
					{
						$arTemplates = CComponentUtil::GetTemplatesList('bitrix:main.banner.view');
